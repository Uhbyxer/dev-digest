...
export function PRRow({ pr, repoId }: { pr: PrMeta; repoId: string }) {
  ...
  // Popover anchor in viewport coords + which severity chip is hovered (null =
  // hidden). Fixed positioning keeps the card visible even though the list
  // container clips overflowing children.
  ...
  const previewFindings = React.useMemo(
    () =>
      preview ? latestFindingsPerAgent(reviews ?? []).filter((f) => f.severity === preview.severity) : [],
    [reviews, preview],
  );
  return (
    <div
    ...
    >
      ...
      <div style={s.findingsCell} onMouseLeave={() => setPreview(null)}>
        {hasFindings ? (
        ...
        {preview && previewFindings.length > 0 && (
          <div style={s.findingsPreview(preview.top, preview.left)} onClick={(e) => e.stopPropagation()}>
            <div style={s.findingsPreviewTitle}>
              ...
              {t("list.findingsPreviewTitle", { count: previewFindings.length, severity: preview.severity })}
            </div>
